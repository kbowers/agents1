...
describe('Authentication and Notes API', () => {
  ...
  describe('Authorization', () => {
    ...
    it('should only show user\'s own notes', async () => {
      ...
      const firstUserNoteIds = firstUserResponse.body.notes.map((n: any) => n.id);
      const secondUserNoteIds = secondUserResponse.body.notes.map((n: any) => n.id);
      
      const hasCommonNotes = firstUserNoteIds.some((id: number) => 
        secondUserNoteIds.includes(id)
      );
